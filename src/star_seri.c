...
static void
pack_from(lua_State *L, struct write_block *b, int from, int index) {
...
static void
seri(struct block *b, int len, void **arg, int *size) {
...
star_pack(lua_State *L, void **arg, int *sz, int index) {
	...
	wb_init(&wb, &temp);
	pack_from(L,&wb,0, index);
	assert(wb.head == &temp);
	seri(&temp, wb.len, arg, sz);
